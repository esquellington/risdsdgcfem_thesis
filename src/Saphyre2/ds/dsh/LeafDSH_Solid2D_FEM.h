#ifndef S2_DS_DSH_LEAF_SOLID2D_FEM_H
#define S2_DS_DSH_LEAF_SOLID2D_FEM_H

#include <Saphyre2/ds/dsh/GLeafDSH_Model.h>

#include <Saphyre2/ms/ParticleSystem.h>
#include <Saphyre2/ms/fem/TriangleElementSet2.h>
...
#define __S2D_ENABLE_SAVE_FORCES //TEMP: for Debug/Viz purposes
...
#define __S2D_ENABLE_VIZ_DEGENERATE_ELEMENTS

#define __S2D_ENABLE_KNC
#ifdef __S2D_ENABLE_KNC
#  define __S2D_S2_DS_FEM_ENABLE_TEST
#  define __S2D_ENABLE_KNC_IN_LS
#  include <Saphyre2/ms/constraint/KinematicNodeConstraintSet2.h>
#endif

#define __S2D_ENABLE_CONTACT
#ifdef __S2D_ENABLE_CONTACT
#  define __S2D_ENABLE_CONTACT_D2K
...
#define __S2D_ENABLE_BVH
#ifdef __S2D_ENABLE_BVH
#  include <Geo/bv/GBoundingVolumeHierarchy.h>
//#  define __S2D_ENABLE_SOLID2D_FEM_TEST_BVH
#endif
...
class LeafDSH_Solid2D_FEM: public GLeafDSH_Model< ms::ParticleSystem2D >
{
public:
    LeafDSH_Solid2D_FEM( uint32 uid, IDynamicSystemHierarchy *p_parent );
    ~LeafDSH_Solid2D_FEM();
...
private:
...
#endif
    {
        ...
        , m_KinematicMode(eKM_Mouse)
          // Integrator
        , m_IntegratorType(eIT_QuasiImplicitEuler_dx)//eIT_FullyImplicitEuler_dx)
        ...
        , m_ContactSolver_PCST(ePCST_Alteration)
        , m_ContactSolver_VCST(eVCST_Reaction)
        ...
        , m_ContactSolver_StatictFriction_Coeff(0.25)
        , m_ContactSolver_DynamicFriction_Coeff(0.2)
...
#endif
        // Viz
        , m_GeoObjectDDF( geo::eODDF_Shape
                          | geo::eSDDF_Boundary
                          /*| geo::eSDDF_Interior
                            | geo::eSDDF_DCR*/ )
            {
        ...
        finline void SetDSH( LeafDSH_Solid2D_FEM *p_dsh )
            {
                m_pDSH = p_dsh;
            }
        ...
        static util::ArchetypeLibrary s_LeafDSH_Solid2D_FEM_Params_ArchetypeLibrary; //TEMP:
#endif
...
private:
    ...
    const geo::MeshSolidShape2* m_pMeshS;

    unsigned int m_NumNodes;
    unsigned int m_NumElements;
    ms::fem::EditableTriangleElementSet2 m_TES;
    ms::IForce2::ICache* m_pTESC;
    ...
    void GEBVD( bvh_type::entry_index_type eid, bvh_type::bv_type& bv ) const;
    bvh_type m_BVH;
    bvh_type m_BVH0; //TEMP
#endif

#ifdef __S2D_ENABLE_KNC
    ms::KinematicNodeConstraintSet2 m_KNCS;
#endif
    ...
    */
    ...
    void ApplyContacts_PositionAlteration( Vec2* vec_pos ) const;
    void ApplyContacts_Penalty( Vec2* vec_forces ) const; //IN-integration
    void ApplyContacts_Reaction( Vec2* vec_v ) const; //IN-integration
    void ApplyContacts_Hack(); //POST-integration
    void ApplyContacts_Friction_dv( const Vec2* vec_vel0, Vec2* vec_vel1 ) const; //POST-integration, FAILS

    // Normal/Friction impulses, Normal uses "reaction constraints"
    void ApplyContacts_NormalImpulse( Vec2* vec_v, Real factor_v_to_impulse ); //Sets cpc.m_LambdaN and applies it to v, factor converts to impulses
    void ApplyContacts_FrictionImpulse( Vec2* vec_v, Real factor_impulse_to_v ) const;

    void UpdateContacts_ActiveSet( const Vec2* vec_vel, const Vec2* vec_acc );
    //@}
...
private:
    ...
    //void Step_ImplicitMidpoint( Real dt );
    //void Step_Newmark( Real dt );
    //@}
    ...
    //@{
    void LS_ZeroKNC( Vec2 *v ) const; //Set to 0 all node-entries corresponding to KNC
    void LS_A_times_d( Real* q, Vec2 *vq,
                       Real factor_M, Real factor_K,
                       const Real* d, const Vec2 *vd ) const;
    void LS_At_times_d( Real* q, Vec2 *vq,
                        Real factor_M, Real factor_K,
                        const Real* d, const Vec2 *vd ) const;
    void LS_AtA_times_d( Real* q, Vec2 *vq,
                         Real* tmp, Vec2 *vtmp, //intermediate result
                         Real factor_M, Real factor_K,
                         const Real* d, const Vec2 *vd ) const;
    void LS_Solve( Real factor_M, Real factor_K, Vec2* vy, Vec2* vb, Real& ls_rel_prec, Real& ls_abs_prec, int& ls_iter ) const;
    //@}
...
private:
    ...
    std::vector<ms::KinematicNodeConstraintSet2::knc_id_type> m_vecTestKNC;
...
#endif // S2_DS_DSH_LEAF_SOLID2D_FEM_H
